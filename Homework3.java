...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    // [  6  6  6  6  6  3  6  3  6  3  3  3  3  3  3  ]
    int spots_filled = 0, i = 0, j = 0;

    while (i < ints.length) {
      if (i == ints.length - 1) {
        if (ints[spots_filled - 1] != ints[i]){
          ints[spots_filled] = ints[i];
          spots_filled++;
        }

        for (int z = spots_filled; z < ints.length; z++) {
          ints[z] = -1;
        }
      ...
      while (ints[i] == ints[j] && j != ints.length - 1) {
        j++;
      }
      ints[spots_filled] = ints[i];
      i = j;
